...
#include <iostream>
#include <iterator>
#include "includes/18.12/vector.h"
#include <map>
#include "includes/18.12/money.h"
#include <time.h>
using namespace std;
...
void Task1()
{
    ...
    cout << "Task1" << endl << "Èñõîäíûé ñëîâàðü" << endl;
...
void Task2()
{
    ...
    cout << "Task 2" << endl << "Èñõîäíûé ñëîâàðü" << endl;
    multimap<float, Money>::iterator p2;
    for (p2 = Mymap.begin(); p2 != Mymap.end(); ++p2)
        cout << p2->first + 1 << ")\t" << p2->second << '\n';

    float sr = srednee(Mymap);
    cout << "Ñðåäíåå àðèôìåòè÷åñêîå = ";
    cout << sr << endl;
    ...
    cout << "Ïîñëå óäàëåíèÿ ýëåìåíòîâ áîëüøå ñðåäíåãî àðèôìåòè÷åñêîãî" << endl;
    int i = 1;
...
void Task3()
{
    ...
    cout << "Max = " << max << endl;
    cout << "Êàæäûé ýëåìåíò âåêòîðà äîìíîæåí íà ìàêñèìàëüíûé" << endl;
    for (int i = 0; i < storage.size(); i++)
    {
        storage[i] *= max;
    }
